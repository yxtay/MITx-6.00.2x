...
class Cluster(object):
    ...
    def distances(self, other):
        return [pointA.distance(pointB) \
                for pointB in other.members() \
                for pointA in self.members()]
    ...
    def singleLinkageDist(self, other):
        ...
        return min(self.distances(other))
    ...
    def maxLinkageDist(self, other):
        ...
        self and the other point is from other. Uses the 
        Euclidean dist between 2 points, defined in Point."""
        return max(self.distances(other))
...
class ClusterSet(object):
    ...
    def delete(self, c):
        """
        remove cluster c from cluster list
        raise ValueError if c not in cluster list
        """
    ...
    def mergeClusters(self, c1, c2):
        ...
        for member in c1.members():
            points.append(member)
        for member in c2.members():
            points.append(member)
        self.add(Cluster(points, c1.pointType))
        self.delete(c1)
        self.delete(c2)
    ...
    def getClusterPairs(self):
        ...
        while members:
            clusterA = members.pop(0)
            for clusterB in members:
    ...
    def findClosest(self, linkage):
        ...
        minDist = None
        minPair = None
        for pair in self.getClusterPairs():
            dist = linkage(pair[0], pair[1])
            if not minPair or dist < minDist:
                minDist = dist
                minPair = pair
        return minPair
