...
def simulationWithoutDrug(numViruses, maxPop, maxBirthProb, clearProb,
    ...
    virusPop = []
    for _ in timeSteps:
        virusPop.append(sum([patient.update() for patient in patients]) / float(numTrials))
    
    # plot
    pylab.plot(timeSteps, virusPop, label = 'SimpleVirus')
    pylab.title('Simulation of SimpleVirus Population without Drugs')
...
class ResistantVirus(SimpleVirus):
    ...
    def isResistantToAll(self, drugs):
        ...
        return all([self.isResistantTo(drug) for drug in drugs])
...
def simulationWithDrug(numViruses, maxPop, maxBirthProb, clearProb, resistances,
    ...
    for t in timeSteps:
        ...
        virusPop.append(sum([patient.update() for patient in patients]) / float(numTrials))
        resistantPop.append(sum([patient.getResistPop(['guttagonol']) \
            for patient in patients]) / float(numTrials))
    
    # plot
    pylab.plot(virusPop, label = 'All Viruses')
    pylab.plot(resistantPop, label = 'Resistant Viruses')
