...
class StandardRobot(Robot):
    ...
    def updatePositionAndClean(self):
        ...
        room = self.room
        newPosition = position.getNewPosition(direction, speed)
        while not room.isPositionInRoom(newPosition):
            direction = random.randrange(360)
            self.direction = direction
            newPosition = position.getNewPosition(direction, speed)
        self.position = newPosition
        room.cleanTileAtPosition(newPosition)
...
#        anim.done()
    ...
    return totalTimeStep / float(num_trials)
